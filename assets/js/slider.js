...
function changeSlideStatus() {
    ...
    for (let i = 1; i < imageTotalNumber + 1; i++) {
        console.log(i);
        if ( i === currentSlide ) {
            let current = document.getElementsByClassName("swiper-pagination-bullet")[currentSlide - 1];
            current.classList.add('target');
        } else {
            let current = document.getElementsByClassName("swiper-pagination-bullet")[i - 1];
...
prevImageElemant.addEventListener('click' , () => {
    if (currentSlide !== 1) {
        currentSlide--;
        mainImageElement.setAttribute('src' , `assets/images/image/president${currentSlide}.jpg`);
        changeSlideStatus();
